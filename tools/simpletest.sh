...
if [ "$CHUTNEY_BOOTSTRAP_TIME" -ge 0 ]; then
    ...
    $ECHO "Running $CHUTNEY_ROUNDS simpletest rounds..."
    while [ "$n_rounds" -lt "$CHUTNEY_ROUNDS" ]; do
        n_rounds=$((n_rounds+1))
        if ! "$CHUTNEY" simpletest "$CHUTNEY_NETWORK"; then
            CHUTNEY_WARNINGS_IGNORE_EXPECTED=false \
                CHUTNEY_WARNINGS_SUMMARY=false \
                "$WARNING_COMMAND"
            "$WARNINGS"
            $ECHO "chutney simpletest $n_rounds/$CHUTNEY_ROUNDS failed"
            exit 1
        fi
        $ECHO "Completed $n_rounds/$CHUTNEY_ROUNDS simpletest rounds."
    done
else
