...
ONIONPERFCMD="onionperf measure --tgen=/home/tom/shadow/src/plugin/shadow-plugin-tgen/build/tgen --oneshot --inet-only"
...
if [ "$CHUTNEY_BOOTSTRAP_TIME" -ge 0 ]; then
    ...
    $ECHO "Running $CHUTNEY_ROUNDS onionperf rounds..."
    while [ "$n_rounds" -lt "$CHUTNEY_ROUNDS" ]; do
        n_rounds=$((n_rounds+1))
        if ! "$CHUTNEY" shellcommand "$CHUTNEY_NETWORK" "$ONIONPERFCMD"; then
            CHUTNEY_WARNINGS_IGNORE_EXPECTED=false \
                CHUTNEY_WARNINGS_SUMMARY=false \
                "$WARNING_COMMAND"
            "$WARNINGS"
            $ECHO "chutney onionperf $n_rounds/$CHUTNEY_ROUNDS failed"
            exit 1
        fi
        $ECHO "Completed $n_rounds/$CHUTNEY_ROUNDS onionperf rounds."
    done
else
